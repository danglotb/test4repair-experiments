...
"""
def classifyAstorPatchesFromProject(t4rpath, betjsonData):
    global errorNoSeed
    ...
    for bugid in os.listdir( patchesFolder):
        ...
        if not os.path.isfile(fbug):
            ...
            a_overfit, b_overfit = classifyAstorPatchesFromBugId(patchesFolder,bugid,betjsonData)
...
'''
def classifyAstorPatchesFromBugId(t4rpath, bugid, betjsonData):
    global errorNoSeed
    ...
    for seed in os.listdir(bugIdResultsFolder):
                ...
                aoverfitseed, boverfitseed = classifyPatchesFromTrial(pathToTrial, bugid, betjsonData)
...
"""
def classifyPatchesFromTrial(repairAttempt, bugid, jsonBetData):
    ...
    if patches is not None and len(patches) > 0:
        ...
        for patch in patches:
            ...
            if any(patchkey in s for s in patchAnalyzed):
                continue
            else:
...
def runExperiment(path,jsonbet, fileoutput = "../../output/patchOverfittingClassification.json"):
    '''given a test4repair project, classifies the patches'''
    allProjectResults = []
    for project in ["Math", "Chart", "Time"]:
        projPath = os.path.join(path,project)
        result =  classifyAstorPatchesFromProject(projPath,jsonbet)
